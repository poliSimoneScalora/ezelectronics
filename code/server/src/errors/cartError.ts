...
const QUANTITY = "Quantity in cart is more than available quantity"
...
/**
 * Represents an error that occurs when a product is in cart with a quantity which is more then the available one.
 */
class ProductQuantityError extends Error {
    customMessage: string
    customCode: number

    constructor() {
        super()
        this.customMessage = QUANTITY
        this.customCode = 409
    }
}
...
export { CartNotFoundError, ProductInCartError, ProductQuantityError, ProductNotInCartError, WrongUserCartError, EmptyCartError }